...
simplifyZero (Op x y z) = if x == Plus && y == (Lit 0) then z
                            else if x == Plus && z == (Lit 0) then y
                            else (Op x y z) 
...
peephole x (Lit a) = (Lit a)
peephole x (Op a b c) = (Op a (peephole x (x b)) (peephole x (x c)))  
...
step (IPush x) y = Just (x:y)
step (IOp x) (a:b:xs) = step (IPush (interp (Op x (Lit b) (Lit a)))) xs
step _ _ = Nothing

--The order of a and b is changed
...
run x y = foldM func y x
    where func a b = step b a    
...
compile :: Expr -> [Instr]
compile (Op a b c) = compile b ++ compile c ++ [(IOp a)]
